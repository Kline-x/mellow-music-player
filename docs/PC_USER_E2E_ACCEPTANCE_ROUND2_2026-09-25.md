...
> 本轮证据目录：`docs/evidence/pc-user-acceptance-round2/`（**67 张本轮实测截图**，已清理纯黑废帧）
...
| P0-3a | 歌手粉丝数为公式编造 | 公式已删除；歌手卡不再渲染「粉丝」行（`05_artists` 与上轮 `04_artists` 对比图见 `docs/evidence/pc-user-acceptance-round2/`） | ✅ 已修复 |
| P0-3b | LX 预设音源返回编造 CDN 直链 | **仍存在**：`lx_script_sandbox.dart:458`（`cdn.<平台>.music.net/media/...`）与 `:794`（`custom-cdn.<id>.com/...`）未被移除，预设驱动仍在注册（`:1271-1287`），且"换源"显式路径仍会调它们（`online_music_service.dart:462 / 508 / 528 / 548 / 580` 以 `enableSourceFallback:false` 命中预设驱动） | ❌ **未修复** |
...
| P1-4 | EQ 不影响音频（滤镜函数无调用者） | 弹窗新增诚实副标题「多频段声音动态补偿与偏好调节 · **高级 DSP 音频滤镜扩展中**」；生产代码 `app/lib` 内无调用者，仅 `app/test` 两处在断言 → 听感仍不受影响；且预设由 5 个减为 4 个（原型的「空间 3D」不存在） | ⚠️ 部分修复（文案已诚实化，功能未接入） |
| P1-5 | macOS 出现 Windows 专有路径/文案 | 本地扫描默认目录改为平台化（`Platform.isWindows ? ... : ~/Music`），提示文案随平台变化 | ✅ 已修复 |
| P1-6 | 收藏跨页面不同步 | 播放栏点红心后「我喜欢的音乐」由 **4 首 → 5 首**（`07_favorites` vs `71_favorites_after`） | ✅ 已修复 |
| V-3 | EQ 弹窗第 4/5 预设被裁 | 4 个预设全部完整显示（原声直通/澎湃低音/通透人声/温润爵士） | ✅ 已修复（以"减少到 4 个"的方式） |
| V-4 | 声音电台页 70% 空白 | 电台页改为自适应网格并填充了两组卡片（`06_podcast`），空白显著减少（数据仍为 `mockRadioStations`） | ⚠️ 视觉改善，数据仍为 mock |
